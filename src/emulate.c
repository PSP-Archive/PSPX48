...
# ifdef DEBUG_INST
  fprintf(stdout, "PC=%x INST=%x %x %x %x %x\n", saturn.PC, 
      read_nibble(saturn.PC    ),
      read_nibble(saturn.PC + 1),
      read_nibble(saturn.PC + 2),
      read_nibble(saturn.PC + 3),
...
schedule(void)
{
...
#ifdef DEBUG_SCHED
  fprintf(stdout, "> schedule called after %ld instructions\n", steps);
#endif
...
#ifdef DEBUG_SCHED
  fprintf(stdout, "next timer 2 step: %ld, event: %ld\n",
  ...
  if (device_check) {
    ...
    if ((sched_display -= steps) <= 0) {
      ...
      fprintf(stdout, "check_device: disp_touched %d\n",
...
#ifdef DEBUG_SCHED
  fprintf(stdout, "next receive: %ld, event: %ld\n",
...
#ifdef DEBUG_SCHED
  fprintf(stdout, "next adjtime: %ld, event: %ld\n",
...
#ifdef DEBUG_SCHED
  fprintf(stdout, "next timer 1 step: %ld, event: %ld\n",
  ...
  if ((sched_statistics -= steps) <= 0) {
    ...
    if (delta_t_1 > 0) {
#if 0
      fprintf(stdout, "I/s = %ld, T1 I/TICK = %d (%ld), T2 I/TICK = %d (%ld)\n",
              saturn.i_per_s, saturn.t1_tick, t1_i_per_tick,
              saturn.t2_tick, t2_i_per_tick);
#else
      fprintf(stdout, "I/s = %ld, T1 I/TICK = %d, T2 I/TICK = %d (%ld)\n",
...
#ifdef DEBUG_SCHED
  fprintf(stdout, "next statistics: %ld, event: %ld\n",
...
#ifdef DEBUG_SCHED
  fprintf(stdout, "next instruction rollover: %ld, event: %ld\n",
...
emulate(void)
{
  ...
  //fprintf(stdout, "Enter in emulation loop\n");
