...
static hp48_timer_t timers[NR_TIMERS];
...
#ifdef DEBUG_TIMER
  fprintf(stdout, "Timer[%d] start at 0x%.8lx%.8lx\n", timer,
...
#ifdef DEBUG_TIMER
  fprintf(stdout, "Timer[%d] restart at 0x%.8lx%.8lx\n", timer,
          timers[timer].start.hi, timers[timer].start.lo);
#endif
}
...
#ifdef DEBUG_TIMER
  fprintf(stdout, "Timer[%d] stop at 0x%.8lx%.8lx, value 0x%.8lx%.8lx\n",
  ...
  if (delta.hi != 0 || (delta.lo > 0x3c000))	/* Half a minute */
    {
      ...
      fprintf(stdout, "Time adjusted by ");
      if (adj_time.hi)
        fprintf(stdout, "%lX%.8lX", adj_time.hi, adj_time.lo);
      else
        fprintf(stdout, "%lX", adj_time.lo);
      fprintf(stdout, " TICKS, Total offset ");
      if (set_0_time.hi)
        fprintf(stdout, "%lX%.8lX", set_0_time.hi, set_0_time.lo);
      else
        fprintf(stdout, "%lX", set_0_time.lo);
      fprintf(stdout, " TICKS\n");
#endif
    }
