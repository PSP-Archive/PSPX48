...
read_rom_file(char *name, unsigned char **mem, unsigned int *size)
{
  ...
  *size = 0;
  ...
  if (fread(four, 1, 4, fp) != 4)
    {
      fprintf(stdout, "can\'t read first 4 bytes of %s\n", name);
      fclose(fp);
      return 0;
    }
  ...
  if (four[0] == 0x02 && four[1] == 0x03 &&
    ...
    {
      fprintf(stdout, "%s is not a HP48 ROM\n", name);
      fclose(fp);
      return 0;
    }
  ...
  if (fseek(fp, 0L, SEEK_SET) < 0)
    {
      fprintf(stdout, "can\'t fseek to position 0 in %s\n", name);
      *size = 0;
      fclose(fp);
      return 0;
    }
  ...
  if (st.st_size == *size)
  ...
  else
    {
      ...
      if (st.st_size != *size / 2)
        {
          fprintf(stdout, "strange size %s, expected %d, found %ld\n",
  ...
  if ((*mem)[0x29] == 0x00)
    {
      if (*size == ROM_SIZE_GX)
        ...
        {
          fprintf(stdout, "%s seems to be G/GX ROM, but size is 0x%x\n",
		  name, *size);
          free(*mem);
          *mem = NULL;
          *size = 0;
  ...
  else
    {
      if (*size == ROM_SIZE_SX)
        ...
        {
          fprintf(stdout, "%s seems to be S/SX ROM, but size is 0x%x\n",
		  name, *size);
          free(*mem);
          *mem = NULL;
          *size = 0;
