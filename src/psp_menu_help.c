...
psp_initialize_help(void)
{
  ...
  snprintf(FileName, MAX_PATH, "%s/help.txt", HP48.psp_home_path);
...
psp_display_screen_help(void)
{
  ...
  if (psp_menu_dirty) {
    ...
    psp_pg_back_print(30, 254, " Arrows: Scroll  Select: Back ", 
  ...
  while ((index < HELP_LINE_BY_PAGE) && (help_line < psp_help_size))  {
    ...
    psp_pg_back_print(x, y, buffer, PSP_MENU_SEL_COLOR);
    y += y_step;
    index++;
    help_line++;
  }
